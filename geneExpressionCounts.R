# This script reads in abundance.tsv files generated by Kallisto, extracts either the est_counts or tpm column from each, collates them 
...
#     argument to ".").
#   3. Whether you'd like to collect the est_counts or the tpm.
...
for(i in 1:length(samples)){
  ...
  if(args[3]=="est_counts"){
    counts <- abundance$est_counts
  }
  if(args[3]=="tpm"){
    counts <- abundance$tpm
  }
  aggregated <- aggregate(counts,by=list(gene.symbols),FUN="sum")
  gene.expression.counts[,i] <- aggregated[,2]
}
